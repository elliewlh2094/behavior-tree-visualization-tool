...
export function ValidationPanel() {
  ...
  return (
    <section
      ...
      className="flex max-h-64 min-h-32 flex-col border-t border-slate-200 bg-white"
    >
      <header className="flex items-center justify-between border-b border-slate-200 px-3 py-2">
        <div className="flex items-center gap-3 text-sm">
          <span className="font-semibold text-slate-900">Validation</span>
      ...
      {sorted.length === 0 ? (
        <div className="px-3 py-2 text-sm text-emerald-700">
        ...
        <ul className="flex-1 divide-y divide-slate-100 overflow-y-auto">
          {sorted.map((issue, idx) => {
            ...
            const rowInteractive = canSelect
              ? 'cursor-pointer hover:bg-slate-50 focus:outline-none focus:bg-sky-50'
              : 'cursor-default';
            return (
              <li key={`${issue.ruleId}-${idx}`}>
                ...
                >
                  ...
                  <span className="min-w-[2.25rem] font-mono text-xs text-slate-500">
                    {issue.ruleId}
                  </span>
                  <span className="flex-1 text-slate-800">{issue.message}</span>
                </button>
              </li>
            );
          })}
